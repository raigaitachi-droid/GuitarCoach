...
class PlayingScreen:
    ...
    def _draw_stage_background(self, surface: pygame.Surface, layout: _Layout) -> None:
        """Draw a sleek studio stage frame and raised track surface."""
        t = get_theme()
        # Top studio header bar
        pygame.draw.rect(surface, (11, 14, 21), (0, 0, layout.screen_w, LANE_TOP_MARGIN))
        pygame.draw.line(surface, (30, 39, 54), (0, LANE_TOP_MARGIN - 1), (layout.screen_w, LANE_TOP_MARGIN - 1))
        ...
        pygame.draw.line(surface, (30, 39, 54), (0, footer_y), (layout.screen_w, footer_y))

        # Fretboard track surface with clean outer border
        track_rect = pygame.Rect(
            14, LANE_TOP_MARGIN - 8,
            layout.screen_w - 28, footer_y - LANE_TOP_MARGIN + 16,
        )
        pygame.draw.rect(surface, (6, 9, 14), track_rect.move(0, 4), border_radius=18)
        pygame.draw.rect(surface, (16, 21, 30), track_rect, border_radius=18)
        pygame.draw.rect(surface, (40, 52, 70), track_rect, 1, border_radius=18)
    ...
    def _draw_lanes(self, surface: pygame.Surface, layout: _Layout) -> None:
        t = get_theme()
        # Realistic string gauges: 1=high E (thin) to 6=low E (thick wound string)
        string_gauges = [1, 2, 2, 3, 3, 4]
        for i in range(6):
            y = LANE_TOP_MARGIN + i * layout.lane_height
            bg = t.lane_bg_even if i % 2 == 0 else t.lane_bg_odd
            lane_rect = pygame.Rect(14, int(y), layout.screen_w - 28, int(layout.lane_height + 1))
            pygame.draw.rect(surface, bg, lane_rect)

            # Centered string line with realistic physical gauge thickness
            line_y = int(y + layout.lane_height / 2)
            gauge = string_gauges[i]
            # Wound strings (D, A, Low E) have subtle bronze/nickel tone
            str_color = (135, 148, 168) if i < 3 else (145, 140, 128)
            pygame.draw.line(
                surface, str_color,
                (58, line_y), (layout.screen_w - 24, line_y),
                gauge,
            )

    def _draw_string_labels(self, surface: pygame.Surface, layout: _Layout) -> None:
        """Draw tuning labels and string numbers with glowing accent rings."""
        t = get_theme()
        scale = max(1.0, min(1.45, layout.screen_h / 720.0))
        font = _get_font("Segoe UI Semibold", int(15 * scale))
        num_font = _get_font("Segoe UI", int(10 * scale))
        for index, label in enumerate(STRING_LABELS):
            ...
            size = int(32 * scale)
            bg_rect = pygame.Rect(20, center_y - size // 2, size, size)

            # Badge background & colored border ring
            pygame.draw.circle(surface, (14, 18, 26), bg_rect.center, size // 2)
            pygame.draw.circle(surface, color, bg_rect.center, size // 2, width=2)

            # String name
            surface.blit(
                text,
                (bg_rect.centerx - text.get_width() // 2,
                 bg_rect.centery - text.get_height() // 2 - 1),
            )

    def _draw_hit_zone(self, surface: pygame.Surface, layout: _Layout) -> None:
        """Draw laser hit zone line with target diamond markers on each string."""
        t = get_theme()
        x = int(layout.hit_zone_x)
        top = int(LANE_TOP_MARGIN)
        bottom = int(LANE_TOP_MARGIN + 6 * layout.lane_height)

        # Soft laser glow gradient
        glow = pygame.Surface((44, bottom - top), pygame.SRCALPHA)
        for width, alpha in ((42, 8), (28, 18), (14, 38)):
            pygame.draw.rect(
                glow, (*t.hit_zone, alpha),
                ((44 - width) // 2, 0, width, bottom - top),
                border_radius=width // 2,
            )
        surface.blit(glow, (x - 22, top))

        # Sharp vertical target line
        pygame.draw.line(surface, t.hit_zone, (x, top), (x, bottom), 3)

        # Target diamond at each string intersection
        for i in range(6):
            sy = int(LANE_TOP_MARGIN + i * layout.lane_height + layout.lane_height / 2)
            diamond = [(x, sy - 5), (x + 5, sy), (x, sy + 5), (x - 5, sy)]
            pygame.draw.polygon(surface, (13, 16, 23), diamond)
            pygame.draw.polygon(surface, t.hit_zone, diamond, width=2)

        # Top cap
        pygame.draw.circle(surface, (13, 16, 23), (x, top), 9)
        pygame.draw.circle(surface, t.hit_zone, (x, top), 6)
    ...
    def _draw_notes(self, surface: pygame.Surface, layout: _Layout) -> None:
        ...
        fret_font_size = min(28, max(16, int(layout.note_h * 0.50)))
        ...
        for note in notes:
            ...
            # Yousician style note rendering: distinct note head + sustain ribbon
            head_w = max(38, fret_text.get_width() + 18)
            has_sustain = w > head_w + 14

            # Draw sustain ribbon extending behind note head if note has duration
            if has_sustain:
                tail_h = int(layout.note_h * 0.44)
                tail_y = int(y + (layout.note_h - tail_h) / 2)
                tail_w = int(w - head_w / 2)
                tail_rect = pygame.Rect(int(x + head_w / 2), tail_y, tail_w, tail_h)
                
                # Translucent sustain tail surface
                tail_surf = pygame.Surface((tail_rect.width, tail_rect.height), pygame.SRCALPHA)
                tail_color = (*color[:3], 90 if past_hit_zone else 145)
                border_color = (*color[:3], 130 if past_hit_zone else 220)
                pygame.draw.rect(tail_surf, tail_color, (0, 0, tail_rect.width, tail_rect.height), border_radius=tail_h // 2)
                pygame.draw.rect(tail_surf, border_color, (0, 0, tail_rect.width, tail_rect.height), width=1, border_radius=tail_h // 2)
                surface.blit(tail_surf, tail_rect.topleft)

            head_rect = pygame.Rect(int(x), int(y), int(head_w if has_sustain else w), int(layout.note_h))

            # Drop shadow
            shadow = head_rect.move(0, 4)
            pygame.draw.rect(surface, (6, 9, 14), shadow, border_radius=NOTE_CORNER_RADIUS)

            # Open string (fret 0) has a distinct ring / aperture styling
            is_open = (note.fret == 0)
            if is_open:
                pygame.draw.rect(surface, (20, 26, 38), head_rect, border_radius=NOTE_CORNER_RADIUS)
                pygame.draw.rect(surface, color, head_rect, width=3, border_radius=NOTE_CORNER_RADIUS)
            else:
                pygame.draw.rect(surface, color, head_rect, border_radius=NOTE_CORNER_RADIUS)
                border = tuple(min(255, channel + 45) for channel in color)
                pygame.draw.rect(surface, border, head_rect, width=2, border_radius=NOTE_CORNER_RADIUS)
                # Glossy top reflection
                highlight = pygame.Rect(head_rect.x + 6, head_rect.y + 4, max(1, head_rect.width - 12), 2)
                pygame.draw.rect(surface, (255, 255, 255), highlight, border_radius=1)

            # Show fret number with high contrast outline
            tx = head_rect.centerx - fret_text.get_width() // 2
            ty = head_rect.centery - fret_text.get_height() // 2
            outline = fret_font.render(fret_label, True, (12, 16, 22))
            surface.blit(outline, (tx + 1, ty + 1))
    ...
    def _draw_hud(self, surface: pygame.Surface, layout: _Layout) -> None:
        ...
        tempo_box = bpm_surf.get_rect(center=(w // 2, 44)).inflate(36, 16)
        pygame.draw.rect(surface, (6, 9, 14), tempo_box.move(0, 3), border_radius=14)
        pygame.draw.rect(surface, (18, 24, 34), tempo_box, border_radius=14)
        pygame.draw.rect(surface, (45, 60, 80), tempo_box, 1, border_radius=14)
