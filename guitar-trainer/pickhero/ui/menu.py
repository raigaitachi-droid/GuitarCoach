    ...
    def render(self, surface: pygame.Surface) -> None:
        ...
        if not files:
            ...
            for i in range(self._scroll_offset, visible_end):
                ...
                if i == self._selected:
                    pygame.draw.rect(surface, (6, 9, 14), card.move(0, 3), border_radius=12)
                    pygame.draw.rect(surface, (24, 34, 48), card, border_radius=12)
                    pygame.draw.rect(surface, t.hud_accent, card, 2, border_radius=12)
                    pygame.draw.rect(surface, t.hud_accent, (card.x, card.y + 10, 4, card.height - 20), border_radius=2)
                    color = t.menu_selected
                else:
                    pygame.draw.rect(surface, (16, 21, 30), card, border_radius=12)
                    pygame.draw.rect(surface, (36, 47, 64), card, 1, border_radius=12)
                    color = t.menu_item
                ...
                marker = (card.x + 28, card.centery)
                pygame.draw.circle(surface, t.hud_accent if i == self._selected else (45, 58, 76), marker, 13)
                play = [(marker[0] - 3, marker[1] - 5), (marker[0] - 3, marker[1] + 5), (marker[0] + 6, marker[1])]
                pygame.draw.polygon(surface, (10, 14, 20), play)
                text_surf = item_font.render(label, True, color)
                surface.blit(text_surf, (card.x + 54, card.centery - text_surf.get_height() // 2))
            ...
            if self._scroll_offset > 0:
                arrow = hint_font.render("▲ more songs", True, t.hud_text)
                surface.blit(arrow, (w // 2 - arrow.get_width() // 2, list_top - 22))
            if visible_end < len(files):
                arrow = hint_font.render("▼ more songs", True, t.hud_text)
                y_bottom = list_top + self._visible_items * item_h + 4
                surface.blit(arrow, (w // 2 - arrow.get_width() // 2, y_bottom))

        # Current audio device pill with active status dot
        dev_box_y = h - 68
        dev_rect = pygame.Rect(content_left, dev_box_y, min(340, content_w // 2), 34)
        pygame.draw.rect(surface, (16, 22, 32), dev_rect, border_radius=10)
        pygame.draw.rect(surface, (38, 50, 68), dev_rect, 1, border_radius=10)
        # Active status dot
        pygame.draw.circle(surface, (46, 213, 115), (dev_rect.x + 16, dev_rect.centery), 4)
        dev_text = f"AUDIO IN:  {self._device_name}"
        dev_surf = hint_font.render(dev_text, True, (215, 225, 238))
        surface.blit(dev_surf, (dev_rect.x + 28, dev_rect.centery - dev_surf.get_height() // 2))
        ...
        if self._search_active:
            ...
            hint = f"[F] Filter  •  [N] Sort ({sort_label})  •  [ENTER] Practice  •  [D] Audio Device  •  [T] Theme  •  [ESC] Quit"
        hint_surf = hint_font.render(hint, True, (135, 150, 172))
        surface.blit(hint_surf, (content_left, h - 24))
